...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $admin_id = (int)$_POST['admin_id'];
     ...
     if (empty($nickName) || strlen($nickName) < 3 || strlen($nickName) > 20) {
     ...
     } elseif ($age === false || $age < 18 || $age > 100) {
         $error = "Вік має бути числом від 18 до 100.";
        ...
        // Ін'єкція
        $stmt = $conn->prepare("INSERT INTO teacher (nickName, firstName, lastName, phone, age, admin_id) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssis", $nickName, $firstName, $lastName, $phone, $age, $admin_id);

        if ($stmt->execute()) {
            echo '<script type="text/javascript">alert("Вчителя успішно додано!"); window.history.back();</script>';
            exit;
        } else {
